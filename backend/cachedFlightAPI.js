...
module.exports = {
    ...
    getFlightData: function(start, end, date) {
      return fetch("https://api-dev.fareportallabs.com/air/api/search/searchflightavailability", {
        ...
        body: JSON.stringify({
          ...
          "FlightSearchRequest": {
            ...
            "SegmentDetails": [
              {
                "DepartureDate": module.exports.formatDate(date),
                "DepartureTime": "1100",
                "Destination": end,
                "Origin": start
              }
    ...
    calculateTripCost: async function(itinerary, currMinCost, startDate, cityList, savedFlights) {
      ...
      for (var i = 0; i < itinerary.length - 1; i++) {
        ...
        if (flightData == -1) {
          ...
          savedFlights[key] = -2;
          var apiData = await module.exports.getFlightData(itinerary[i], itinerary[i+1], date);
          flightData = await module.exports.parseFlightData(apiData);
        }
    ...
    permute: function(list, l, r, permutationsList, startingCity) {
      if (l == r) {
    ...
    // run in parallel
    // compare itineraries
    checkItineraries: async function(permutations, startDate, cityList, savedFlights) {
      const promises = permutations.map(itinerary => { return module.exports.calculateTripCost(itinerary, Number.MAX_SAFE_INTEGER, startDate, cityList, savedFlights)});
    ...
    getOptimalPath: async function(permutations, startDate, cityList, savedFlights) {
      // optimal path
    ...
    processInput: async function(requestJSON) {
      // parse JSON
      ...
      var itineraries = [];
      module.exports.permute(cities, 0, cities.length-1, itineraries, startingCity);
      
      // shuffle array to improve cache hits
      itineraries = module.exports.shuffle(itineraries);

      // get optimal path
      var savedFlights = {};
      cityList[startingCity] = 0;
      //var result = await getOptimalPath(itineraries, startDate, cityList, savedFlights);
      var prices = await module.exports.checkItineraries(itineraries, startDate, cityList, savedFlights);
